...
class VideosBloc extends Bloc<VideosEvent, VideosState> {
  ...
  Future<void> _onRefreshVideos(
    ...
    result.fold(
      ...
      (videos) {
        ...
        emit(_applyFiltersAndSort(loaded as VideosLoaded));
      },
    );
  }
  ...
  void _onFilterByChannel(
    ...
    final currentState = state;
    if (currentState is VideosLoaded) {
      final updated = currentState.copyWith(selectedChannel: event.channelName);
  ...
  void _onFilterByCountry(
    ...
    final currentState = state;
    if (currentState is VideosLoaded) {
      final updated = currentState.copyWith(selectedCountry: event.country);
      emit(_applyFiltersAndSort(updated));
    }
  }

  void _onSortVideos(
    SortVideosEvent event,
    Emitter<VideosState> emit,
  ) {
    final currentState = state;
    if (currentState is VideosLoaded) {
